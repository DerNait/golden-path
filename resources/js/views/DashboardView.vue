...
<template>
  <div>
    ...
    <template v-else-if="data">
      ...
      <section class="surface panel section-block"><div class="section-heading"><div><h2>{{ data.active_phase?.name || 'Fase de entrenamiento' }}</h2><p v-if="data.active_phase">{{ new Date(`${data.active_phase.starts_on}T12:00:00`).toLocaleDateString('es-GT') }} - {{ new Date(`${data.active_phase.ends_on}T12:00:00`).toLocaleDateString('es-GT') }}</p></div><span class="badge-soft green">{{ data.active_phase?.minimum_target_sessions || 40 }} sesiones meta</span></div><div class="grid stats-grid"><div><small class="text-secondary d-block">Peso reciente</small><strong>{{ data.latest_measurement?.body_weight_kg ?? data.profile.current_body_weight_kg ?? 'Pendiente' }} <small v-if="data.latest_measurement?.body_weight_kg || data.profile.current_body_weight_kg">kg</small></strong></div><div><small class="text-secondary d-block">Cintura reciente</small><strong>{{ data.latest_measurement?.waist_cm ?? data.profile.current_waist_cm ?? 'Pendiente de registrar' }} <small v-if="data.latest_measurement?.waist_cm || data.profile.current_waist_cm">cm</small></strong></div><div v-if="data.latest_measurement?.body_fat_percentage!=null"><small class="text-secondary d-block">Grasa corporal</small><strong>{{ data.latest_measurement.body_fat_percentage }} <small>%</small></strong></div><div v-if="data.latest_measurement?.skeletal_muscle_mass_kg!=null"><small class="text-secondary d-block">Musculo esqueletico</small><strong>{{ data.latest_measurement.skeletal_muscle_mass_kg }} <small>kg</small></strong></div><div><small class="text-secondary d-block">Proteina orientativa</small><strong>{{ data.profile.protein_goal_min_grams }}-{{ data.profile.protein_goal_max_grams }} g</strong></div><div><small class="text-secondary d-block">Sueno orientativo</small><strong>{{ data.profile.sleep_goal_hours }}+ h</strong></div></div></section>

      <section class="grid two-column section-block">
        <div><div class="section-heading"><div><h2>Recomendaciones de la IA</h2><p>Aplicadas en cuanto tu entrenador las publica.</p></div><RouterLink class="icon-button compact" title="Ver progreso" :to="{name:'progress'}"><i class="fa-solid fa-arrow-right"/></RouterLink></div>
          <div v-if="data.recent_recommendations.length" class="recommendation-list"><article v-for="item in data.recent_recommendations" :key="item.id" class="recommendation-row"><div class="d-flex justify-content-between gap-2"><strong>{{ item.exercise.name }}<small v-if="item.target_sets" class="text-secondary"> · {{ item.target_sets }} series</small></strong><span class="d-flex gap-1"><span class="badge-soft green">IA</span><span class="badge-soft blue">{{ confidenceLabel(item.confidence) }}</span></span></div><p class="small text-secondary mb-0 mt-2">{{ item.reason }}</p></article></div>
          <EmptyState v-else title="Sin recomendaciones de la IA" text="Pidele a tu entrenador IA un analisis para recibir recomendaciones." icon="fa-lightbulb" />
        </div>
